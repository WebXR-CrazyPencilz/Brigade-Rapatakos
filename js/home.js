...
window.HomeModule = (function () {
  ...
  function injectHTML() {
    ...
    style.textContent = `
      ...
      /* ── Carousel ── */
      #carousel { position:fixed; inset:0; bottom:62px; background:#0a0805; overflow:hidden; }
      #carousel-track {
      ...
      .c-slide { flex-shrink:0; width:100vw; height:100%; display:flex; align-items:center; justify-content:center; }
      .c-slide img { width:100%; height:100%; object-fit:cover; display:block; pointer-events:none; }
      .c-placeholder { font-family:'Cormorant Garamond',serif; font-size:100px; font-weight:300; color:rgba(200,190,154,.06); }
      ...
      .c-arrow:hover { background:rgba(200,190,154,.18); border-color:rgba(200,190,154,.65); }
      .c-arrow svg { width:14px; height:14px; stroke:rgba(200,190,154,.80); fill:none; stroke-width:2; stroke-linecap:round; stroke-linejoin:round; }
      #c-prev { left:14px; }
      #c-next { right:14px; }

      /* Dots */
      #c-dots { position:absolute; bottom:16px; left:50%; transform:translateX(-50%); display:flex; gap:8px; pointer-events:none; }
      ...
      #lb-empty {
        ...
        color:rgba(200,190,154,.35); font-size:16px;
      }
      #lb-close {
      ...
      #lb-zoom-hint {
        ...
        color:rgba(200,190,154,.25); pointer-events:none;
    ...
    document.body.insertAdjacentHTML('beforeend', `
      ...
      <div id="carousel">
        ...
        <button class="c-arrow" id="c-prev" aria-label="Previous">
          <svg viewBox="0 0 24 24"><polyline points="15 18 9 12 15 6"/></svg>
        </button>
        <button class="c-arrow" id="c-next" aria-label="Next">
      ...
      <div id="lightbox">
        ...
        <div id="lb-close">✕</div>
        <div id="lb-zoom-hint">Pinch or scroll to zoom</div>
      </div>
  ...
  function initCarousel() {
    ...
    document.getElementById('c-prev').addEventListener('click', (e) => { e.stopPropagation(); goTo(current - 1); startAuto(); });
    document.getElementById('c-next').addEventListener('click', (e) => { e.stopPropagation(); goTo(current + 1); startAuto(); });
  ...
  // ─── LIGHTBOX + ZOOM ─────────────────────────────────────────────
  let lbScale = 1;
  let lbOrigin = { x: 0, y: 0 };   // pan offset
  ...
  function lbApplyTransform(smooth) {
    ...
    img.style.transform  = `scale(${lbScale}) translate(${lbOrigin.x / lbScale}px, ${lbOrigin.y / lbScale}px)`;
  }

  function lbResetZoom() {
    lbScale = 1; lbOrigin = { x: 0, y: 0 };
  ...
  function bindLightboxZoom() {
    ...
    lb.addEventListener('wheel', (e) => {
      ...
      if (lbScale === LB_MIN) lbOrigin = { x: 0, y: 0 };
    ...
    lb.addEventListener('touchstart', (e) => {
      ...
      if (activeTouches.length === 2) {
        ...
        lbPanStart = { x: activeTouches[0].clientX - lbOrigin.x, y: activeTouches[0].clientY - lbOrigin.y };
      }
    }, { passive: true });
    ...
    lb.addEventListener('touchmove', (e) => {
      ...
      if (activeTouches.length === 2) {
        ...
        lbScale = Math.min(LB_MAX, Math.max(LB_MIN, lbScale * ratio));
        lbPinchDist = dist;
        if (lbScale === LB_MIN) lbOrigin = { x: 0, y: 0 };
        lbApplyTransform(false);

      } else if (activeTouches.length === 1 && lbScale > 1) {
        // Pan when zoomed
        lbOrigin.x = activeTouches[0].clientX - lbPanStart.x;
        lbOrigin.y = activeTouches[0].clientY - lbPanStart.y;
        lbApplyTransform(false);
      }
    }, { passive: false });
    ...
    lb.addEventListener('touchend', (e) => {
      activeTouches = Array.from(e.touches);
      if (activeTouches.length < 2 && lbScale > 1) {
    ...
    let lastTap = 0;
    lb.addEventListener('dblclick', lbResetZoom);
    lb.addEventListener('touchend', (e) => {
      const now = Date.now();
      if (now - lastTap < 280 && e.changedTouches.length === 1) lbResetZoom();
      lastTap = now;
    });

    // Close
    document.getElementById('lb-close').addEventListener('click', closeLightbox);
    lb.addEventListener('click', (e) => { if (e.target === lb || e.target === wrap) closeLightbox(); });
  ...
  function bindPanelEvents() {
    ...
    document.addEventListener('click', (e) => {
      ...
      const fpOverlay = document.getElementById('fp-overlay');
      const lb        = document.getElementById('lightbox');
      if (lb && lb.classList.contains('open')) return; // lightbox handles its own closing
      const clickedOutside =
        bar && row &&
        !bar.contains(e.target) && !row.contains(e.target) &&
